...
import { getAccessToken } from "./lib/msalClient.mjs";
import { uploadFile, getMyDisplayName } from "./lib/graph.mjs";
 ...
 * own OneDrive, and (optionally) a full JSON export of the applicants
 * table -- an independent, self-owned copy of the data that survives even
 * if access to this Supabase project is ever lost. Incremental: every
 * upload is recorded in external_backups, so re-running only touches
 * files that are missing or previously failed.
 *
 * Usage:
...
const rootFolder = process.env.ONEDRIVE_ROOT_FOLDER || "RecruitmentBackups";
...
async function main() {
  ...
  console.log(`${totalCandidates} file(s) total, ${pending.length} not yet backed up to OneDrive.`);
  ...
  const who = await getMyDisplayName(accessToken);
  console.log(`Uploading to OneDrive account: ${who || "(unknown)"}\n`);
  ...
  for (const item of batch) {
    ...
    try {
      ...
      const remotePath = `${rootFolder}/${item.kind}/${item.applicantId}/${basename(item.path)}`;
      const uploaded = await uploadFile(accessToken, remotePath, buffer);
  ...
  if (exportData) {
    ...
    const remotePath = `${rootFolder}/data-exports/applicants-${dateStr}.json`;
    const uploaded = await uploadFile(accessToken, remotePath, Buffer.from(json, "utf-8"));
