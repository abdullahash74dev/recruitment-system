...
BEGIN
  ...
  v_hidden := COALESCE(ARRAY(SELECT jsonb_array_elements_text(v_prefs->'hiddenCandidateIds'))::uuid[], ARRAY[]::uuid[]);
  ...
  rejection AS (
    SELECT COALESCE(rr.reason_ar,'—') AS name,
     ...
     WHERE c.status::text = 'rejected' AND NOT (c.id = ANY(v_hidden))
  ...
  accepted_list AS (
    SELECT c.id, c.full_name, c.nationality, c.status::text AS status,
     ...
     WHERE c.status::text IN ('selected','offer_sent','offer_signed','offer_accepted','hired','started')
       AND NOT (c.id = ANY(v_hidden))
     ORDER BY c.created_at DESC
  ),
  rejected_list AS (
    SELECT c.id, c.full_name, c.nationality,
